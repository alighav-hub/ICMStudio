// This file will contain all the JavaScript logic for the ICM Studio application.
// The original ICM Studio JavaScript code should be placed here.
...
(() => {
  ...
  function extendImageEdges(ctx, srcCanvas, blurRadius) {
    ...
    // bottom-right
    ctx.drawImage(srcCanvas, w - 1, h - 1, 1, 1, blurRadius + h, blurRadius + h, blurRadius, blurRadius);
  ...
  function redrawOutput() {
    if (!originalImage || !resizedImageBitmap) {
      ...
      if (uploadHint) uploadHint.style.display = 'block'; // Ensure hint is visible when no image
      return;
    } else {
        if (uploadHint) uploadHint.style.display = 'none'; // Hide hint when image is present
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
      ...
      drawSplineEditor();
      autoSizeCanvases();
    }, 100); // Defer initialization to allow layout to settle
  });
})();
